...
class DQN(nn.Module):
    ...
    def __init__(self, h, w, num_actions):
        ...
        ### WRITE YOUR CODE BELOW HERE
        def covl_output(input,kernel,stride):
             return ((input-kernel)//stride)+1
        self.conv1 = nn.Conv2d(3, 16, 5, 2)
        self.bn1 = nn.BatchNorm2d(16)
        h_out = covl_output(h,5,2)
        w_out = covl_output(w,5,2)

        self.conv2 = nn.Conv2d(16, 32, 5, 2)
        self.bn2 = nn.BatchNorm2d(32)
        h_out = covl_output(h_out,5,2)
        w_out = covl_output(w_out,5,2)
        ...
        self.head = nn.Linear(h_out*w_out*32, num_actions)
...
def select_action(state, policy_net, num_actions, epsilon, steps_done = 0, bootstrap_threshold = 0):
    ...
    if steps_done>bootstrap_threshold:
      new_epsilon = new_epsilon-0.0005
...
def doMakeBatch(replay_memory, batch_size):
    ...
    for s in batch.next_state:
      if s is not None:
        list_next_s.append(s)

    next_states_batch = torch.cat(list_next_s)
    list_final = []
    for s in batch.next_state:
      if s is not None:
        list_final.append(1)
        ...
        list_final.append(0)
    non_final_mask = torch.tensor(tuple(list_final), device=DEVICE, dtype=torch.bool)
